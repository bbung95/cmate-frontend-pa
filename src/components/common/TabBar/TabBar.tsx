...
export default function TabBar({ children, className = '' }: TabBarProps) {
	return (
		<nav
			className={`border-gray-4 bg--white fixed right-0 bottom-0 left-0 z-50 flex h-[var(--tab-bar-height)] items-center border-t bg-white px-16 pt-7 pb-30 ${className}`}
		>
			{children}
		</nav>
	);
